...
export const useGameStore = create<GameStore>((set) => ({
  ...
  units: [
    ...
    // Enemy starting villagers (visible from frame 0 so the world feels alive
    // before the AI tick spawns its first wave at +10s).
    {
      id: crypto.randomUUID(),
      type: 'villager',
    ...
    },
    {
      ...
      damage: getUnitCombatStats('stone', 'warrior').damage,
      speed: getUnitCombatStats('stone', 'warrior').speed,
      range: getUnitCombatStats('stone', 'warrior').range,
      attackSpeed: getUnitCombatStats('stone', 'warrior').attackSpeed,
      lastAttackTime: 0,
    },
  ],
  buildings: [
