...
function getEffectiveUnitDamage(attacker: Unit, target: Unit): number {
  ...
  // Prevent extreme burst in clumped fights.
...
function getEffectiveBuildingDamage(attacker: Unit): number {
  // Buildings should take slower chip damage than units to improve pacing.
...
function getDropOffMultiplier(
  ...
  const hasDropOff = buildings.some(
    (b) => b.owner === owner && b.type === dropOffType && b.hp > 0
  );
...
export const useGameStore = create<GameStore>((set) => ({
  ...
  productionQueues: [
    {
      buildingId: INITIAL_TOWN_CENTER_ID,
      queue: [],
      rallyPoint: null,
    },
  ],
  ...
  addResource: (r, amount) =>
    set((s) => ({ resources: { ...s.resources, [r]: s.resources[r] + amount } })),
    ...
    set((s) => {
      ...
      if (s.mission.type === 'survival' && nextWaves >= 10) {
        return {
          ...
          resources: grantMissionRewards(s),
        };
      }
  ...
  evaluateMission: () =>
    set((s) => {
      ...
      if (s.mission.type === 'conquest') {
        const enemyTownCenterAlive = s.buildings.some(
          (b) => b.owner === 'enemy' && b.type === 'townCenter'
        );
        if (!enemyTownCenterAlive) {
          return {
            missionStatus: 'success',
            resources: grantMissionRewards(s),
          };
      ...
      return {
        ...
        buildings: [
          ...s.buildings,
          {
            ...
            position: { x, y },
            owner: 'player',
            hp: getBuildingMaxHp(type),
            maxHp: getBuildingMaxHp(type),
          },
        ],
        productionQueues: [
          ...s.productionQueues,
  ...
  spawnUnit: (buildingId, unitType) =>
    set((s) => {
      ...
      if (building.owner !== 'player') return s;
      ...
      const nextQueue: ProductionQueue = existingQueue
        ? {
            ...
            queue: [
              ...existingQueue.queue,
              { type: unitType, progress: 0, totalTime: training.totalTime },
            ],
          }
        : {
            buildingId,
            queue: [{ type: unitType, progress: 0, totalTime: training.totalTime }],
            rallyPoint: null,
          };
      return {
        resources: {
          ...s.resources,
          food: s.resources.food - foodCost,
          gold: s.resources.gold - goldCost,
        },
        productionQueues: existingQueue
          ? s.productionQueues.map((q) => (q.buildingId === buildingId ? nextQueue : q))
          : [...s.productionQueues, nextQueue],
      };
    }),
  ...
  setRallyPoint: (buildingId, point) =>
    set((s) => {
      ...
      if (exists) {
        return {
          productionQueues: s.productionQueues.map((q) =>
            q.buildingId === buildingId ? { ...q, rallyPoint: point } : q
          ),
        };
      }
      return {
        productionQueues: [...s.productionQueues, { buildingId, queue: [], rallyPoint: point }],
      };
    }),
  ...
  dealDamage: (attackerId, targetId) =>
    set((s) => {
      ...
      if (targetUnit) {
        ...
        const inRange = distance(attacker.position, targetUnit.position) <= effectiveRange;
        if (!inRange) return s;
        const nextUnits = s.units
          .map((u) => {
      ...
      const effectiveRange = attacker.range * getTerrainRangeMultiplier(s, attacker.position);
      const inRange = distance(attacker.position, buildingCenter) <= effectiveRange + 10;
      if (!inRange) return s;
      const nextUnits = s.units.map((u) => (u.id === attacker.id ? { ...u, lastAttackTime: now } : u));
      const nextBuildings = s.buildings
        .map((b) => (b.id === targetBuilding.id ? { ...b, hp: b.hp - getEffectiveBuildingDamage(attacker) } : b))
        .filter((b) => b.hp > 0);
  ...
  moveCamera: (dx, dy) =>
    set((s) => {
      ...
      return {
        camera: {
          x: Math.max(0, Math.min(s.camera.x + dx, maxX)),
          y: Math.max(0, Math.min(s.camera.y + dy, maxY)),
        },
      };
    }),
  ...
  tickEconomy: () =>
    set((s) => {
      ...
      const foodSupportMult = 1 + farmBonus;
      const mineSupportMult = 1 + mineBonus;
      ...
      if (
        ...
        s.mission.type === 'economy' &&
        nextResources.gold >= 1000 &&
        s.missionElapsedSec <= 300
      ) {
        return {
          resources: {
    ...
    set((s) => ({
      units: s.units.map((u) => {
        ...
        if (u.owner === 'enemy' && !u.target) {
          ...
          if (nextTile && TERRAIN_EFFECTS[nextTile.type].impassable) {
            return u;
          }
          return {
            ...u,
        ...
        if (!unit.path || unit.path.length < 2 || unit.pathIndex === undefined) {
          ...
          if (path && path.length > 1) {
            unit = { ...unit, path, pathIndex: 1 };
          } else {
            return { ...unit, target: undefined, path: undefined, pathIndex: undefined };
          }
        }
  ...
  tickProductionQueues: (dt) =>
    set((s) => {
      let units = [...s.units];
      let changed = false;
      const nextQueues = s.productionQueues
        .filter((q) => s.buildings.some((b) => b.id === q.buildingId))
        .map((q) => {
          if (q.queue.length === 0) return q;
          const head = q.queue[0];
          const progressed = { ...head, progress: head.progress + dt };
          if (progressed.progress < progressed.totalTime) {
            return { ...q, queue: [progressed, ...q.queue.slice(1)] };
          }
    ...
    set((s) => {
      ...
      for (let i = 0; i < nextUnits.length; i++) {
        ...
        if (bestUnitIndex >= 0) {
          ...
          nextUnits[bestUnitIndex] = {
            ...target,
            hp: target.hp - getEffectiveUnitDamage(attacker, target) * terrainDefense,
          };
          landedHits += 1;
          changed = true;
          continue;
        ...
        if (bestBuildingIndex >= 0) {
          ...
          nextUnits[i] = { ...attacker, lastAttackTime: now };
          nextBuildings[bestBuildingIndex] = {
            ...target,
            hp: target.hp - getEffectiveBuildingDamage(attacker),
          };
          landedHits += 1;
          changed = true;
        }
      ...
      const nextShake = hadMoreBuildings
        ? {
            intensity: 7,
            duration: 0.25,
            timer: 0.25,
      ...
      const conquestCleared =
        s.missionStatus === 'active' && s.mission.type === 'conquest' && !enemyTownCenterAlive;
      const nextResources = conquestCleared
        ? {
            food: s.resources.food + s.mission.rewards.food,
            wood: s.resources.wood + s.mission.rewards.wood,
            stone: s.resources.stone + s.mission.rewards.stone,
            gold: s.resources.gold + s.mission.rewards.gold,
          }
        : s.resources;
      return {
        ...s,
  ...
  tickFogOfWar: () =>
    set((s) => {
      ...
      for (let i = 0; i < nextTiles.length; i++) {
        if (nextTiles[i] === 2) nextTiles[i] = 1;
      }
      ...
      const revealSources = [
        ...
        ...s.buildings
          ...
          .map((b) => ({ x: b.position.x + 16, y: b.position.y + 16 })),
      ];
